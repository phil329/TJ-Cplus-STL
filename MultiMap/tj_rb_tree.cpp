...
inline void tj__rb_tree_rebalance(node<T>* x, node<T>*& root) {
	x->color = __rb_tree_red__;		// 新节点比为红色
	while (x != root && x->parent->color == __rb_tree_red__) { // 父节点为红色
		if (x->parent == x->parent->parent->left) { // 父节点为祖父节点的左孩子
			node<T>* y = x->parent->parent->right;	// 令y 为伯父节点
			if (y && y->color == __rb_tree_red__) { 		// 伯父节点存在，且为红色
				x->parent->color = __rb_tree_black__;  		// 更改父节点为黑色
				y->color = __rb_tree_black__;				// 更改伯父节点为黑色
				x->parent->parent->color = __rb_tree_red__; 	// 更改祖父节点为红色
				x = x->parent->parent;
			}
				...
				x->parent->color = __rb_tree_black__;	// 改变颜色，父节点为黑色
			...
			if (y && y->color == __rb_tree_red__) {		// 有伯父节点且为红色
				x->parent->color = __rb_tree_black__;		// 更改父节点为黑色
				y->color = __rb_tree_black__; 				// 更改伯父节点为黑色
				x->parent->parent->color = __rb_tree_red__; 	// 更改祖父节点为红色
	...
	root->color = __rb_tree_black__;	// 根节点永远为黑色
}
